...
import { Sparkles, Film, Sun, Moon, Globe, FolderKanban, Wand2 } from 'lucide-react';

export default function Header({ onGoHome, onOpenProjects, onStartWizard, lang = 'en', theme = 'dark', onToggleLang, onToggleTheme }) {
  const isAr = lang === 'ar';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3 cursor-pointer" onClick={onGoHome}>
          ...
          <span className="hidden sm:inline-block text-xs font-semibold uppercase tracking-wider text-purple-300/80 bg-purple-950/50 theme-light:bg-purple-100 theme-light:text-purple-700 px-2.5 py-1 rounded-full border border-purple-500/20">
            {isAr ? 'منصة الترجمة الأنمي' : 'Anime Fansub Hub'}
          </span>
        </div>
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-slate-900/80 theme-light:bg-slate-100 border border-slate-800 theme-light:border-slate-300 text-purple-300 theme-light:text-purple-700 hover:border-purple-500/40 transition"
          ...
          {/* My Projects Button (Replaces New Project) */}
          <button
            onClick={onOpenProjects}
            className="flex items-center space-x-2 text-xs sm:text-sm font-semibold text-white bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-90 px-3.5 py-2 rounded-xl transition-all shadow-md shadow-purple-500/20"
          >
